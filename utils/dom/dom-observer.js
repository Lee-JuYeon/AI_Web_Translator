...
function initIntersectionObserver() {
  ...
  try {
    ...
    console.log(`[${APP_CONFIG.appName}] IntersectionObserver 초기화됨`);
  } catch (error) {
...
function initMutationObserver() {
  ...
  try {
    ...
    if (document.body) {
      ...
      console.log(`[${APP_CONFIG.appName}] MutationObserver 초기화됨`);
    } else {
      console.warn(`[${APP_CONFIG.appName}] document.body가 준비되지 않았습니다.`);
    }
  } catch (error) {
...
function handleIntersection(entries) {
  try {
    ...
    console.log(`[${APP_CONFIG.appName}] ${visibleEntries.length}개 요소가 화면에 보임`);
...
function handleMutation(mutations) {
  try {
    ...
    console.log(`[${APP_CONFIG.appName}] DOM 변경 감지됨, 새 텍스트 컨테이너 검색`);
...
function observeElement(element) {
  try {
    ...
    if (!state.intersectionObserver) {
      ...
      if (!state.intersectionObserver) {
        console.error(`[${APP_CONFIG.appName}] IntersectionObserver 초기화 실패`);
        return;
      }
    }
...
function observeElements(elements) {
  try {
    ...
    console.log(`[${APP_CONFIG.appName}] ${elements.length}개 요소 관찰 등록`);
  } catch (error) {
...
function setEnabled(isEnabled) {
  ...
  console.log(`[${APP_CONFIG.appName}] DOM 관찰자 ${state.isEnabled ? '활성화' : '비활성화'}`);
}
...
function initialize() {
  try {
    ...
    console.log(`[${APP_CONFIG.appName}] DOM 관찰자 초기화 완료`);
    return true;
  } catch (error) {
 ...
 * @param {Function} findTextContainers - 텍스트 컨테이너 찾기 함수
 * @returns {Element[]} - 화면에 보이는 요소 배열
 */
function processVisibleElements(findTextContainers) {
  try {
    // 텍스트 컨테이너 요소들 찾기
    if (!findTextContainers || typeof findTextContainers !== 'function') {
      console.error(`[${APP_CONFIG.appName}] 텍스트 컨테이너 찾기 함수가 필요합니다.`);
      return [];
    }
    ...
    containers.forEach(element => {
      try {
        ...
        console.warn(`[${APP_CONFIG.appName}] 요소 가시성 확인 오류:`, elementError);
      }
    });
    
    console.log(`[${APP_CONFIG.appName}] 현재 화면에 보이는 ${visibleElements.length}개 요소 처리`);
...
function cleanup() {
  try {
    ...
    console.log(`[${APP_CONFIG.appName}] DOM 관찰자 정리 완료`);
  } catch (error) {
...
function cleanupObserver(observer) {
  if (observer) {
    try {
      ...
      console.warn(`[${APP_CONFIG.appName}] 관찰자 해제 오류:`, observerError);
    }
  }
}
...
function handleError(message, error) {
  ...
  // 오류 이벤트 발행
  try {
    safeDispatchEvent('domobserver:error', {
      message,
      error: error.message
    });
  } catch (eventError) {
    console.warn(`[${APP_CONFIG.appName}] 오류 이벤트 발행 실패:`, eventError);
  }
}
