...
function matchesSelector(element, selector) {
  try {
    ...
    console.warn(`[${APP_CONFIG.appName}] matches 함수 오류:`, matchError);
    return false;
  }
}
...
function hasMinimumTextContent(element) {
  try {
    ...
    console.warn(`[${APP_CONFIG.appName}] textContent 접근 오류:`, textContentError);
    return false;
  }
}
...
function checkTextNodes(element) {
  try {
    ...
    console.warn(`[${APP_CONFIG.appName}] 자식 노드 확인 오류:`, childrenError);
    return false;
  }
}
...
function checkTextAttributes(element) {
  try {
    ...
    console.warn(`[${APP_CONFIG.appName}] 속성 확인 오류:`, attrError);
    return false;
  }
}
  ...
  try {
    ...
    while ((node = walker.nextNode())) {
      try {
        ...
        console.warn(`[${APP_CONFIG.appName}] 텍스트 노드 처리 오류:`, nodeError);
      }
    }
  } catch (walkerError) {
    console.error(`[${APP_CONFIG.appName}] TreeWalker 오류:`, walkerError);
  }
}
...
function extractTextAttributes(element, textNodes) {
  ...
  try {
    ...
    console.error(`[${APP_CONFIG.appName}] 속성 처리 오류:`, attrsError);
  }
}
...
function extractAttributeFromElement(element, attr, textNodes) {
  try {
    ...
    console.warn(`[${APP_CONFIG.appName}] 속성 '${attr}' 처리 오류:`, attrError);
  }
}
...
function extractAttributesFromChildren(element, attr, textNodes) {
  try {
    ...
    console.warn(`[${APP_CONFIG.appName}] 속성 선택자 오류:`, querySelectorError);
  }
}
  ...
  try {
    ...
    elements.forEach(element => {
      try {
        ...
        console.warn(`[${APP_CONFIG.appName}] 컨테이너 요소 처리 오류:`, elementError);
      }
    });
  } catch (querySelectorError) {
    console.error(`[${APP_CONFIG.appName}] querySelector 오류:`, querySelectorError);
  }
}
...
function findElements(selector) {
  try {
    ...
    if (!isValidSelector(selector)) {
      console.warn(`[${APP_CONFIG.appName}] 잘못된 선택자: ${selector}`);
      return [];
    }
...
function extractAllTextNodes() {
  ...
  try {
    ...
    console.log(`[${APP_CONFIG.appName}] 전체 페이지에서 ${textNodes.length}개 텍스트 노드 추출됨`);
  } catch (error) {
  ...
  try {
    ...
    if (selector.includes('(') || selector.includes(')') || 
      ...
      try {
        ...
        console.warn(`[${APP_CONFIG.appName}] 복잡한 선택자 검증 실패: ${selector}`, e);
        return false;
      }
    }
    ...
    console.warn(`[${APP_CONFIG.appName}] 유효하지 않은 선택자: ${selector}`, error);
    return false;
  }
}
...
function safeMatches(element, selector) {
  ...
  try {
    ...
    console.warn(`[${APP_CONFIG.appName}] matches 함수 오류:`, error);
    return false;
  }
}
...
function logSettingsChanges(oldSettings) {
  ...
  if (Object.keys(changedSettings).length > 0) {
    console.log(`[${APP_CONFIG.appName}] DOMSelector 설정 변경:`, changedSettings);
  }
}
...
function resetAllTranslationAttributes() {
  try {
    ...
    console.log(`[${APP_CONFIG.appName}] ${resetCount}개 번역 속성 초기화 완료`);
    return resetCount;
  } catch (error) {
  ...
  try {
    ...
    elements.forEach(element => {
      try {
        ...
        console.warn(`[${APP_CONFIG.appName}] 속성 제거 오류:`, attrError);
      }
    });
    
    return count;
  } catch (error) {
    console.warn(`[${APP_CONFIG.appName}] 속성 리셋 오류:`, error);
    return 0;
  }
}
...
function handleError(message, error) {
  ...
  // 오류 이벤트 발행
  try {
    safeDispatchEvent('domselector:error', {
      message,
      error: error.message
    });
  } catch (eventError) {
