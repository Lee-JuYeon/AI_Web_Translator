...
const DOMHandler = (function() {
  ...
  function checkDependencies() {
    // 필수 모듈
    const dependencies = [
      { name: 'DOMSelector', obj: window.DOMSelector },
      { name: 'DOMObserver', obj: window.DOMObserver },
      { name: 'DOMManipulator', obj: window.DOMManipulator },
      { name: 'BatchEngine', obj: window.BatchEngine }
    ];
      ...
      elements.forEach(element => {
        try {
          if (element instanceof Element) {
            ...
            // 요소에서 텍스트 노드 추출
            const textNodes = window.DOMSelector.extractTextNodesFromElement(element);
  ...
  function handleElementsAdded(event) {
    try {
      ...
      elements.forEach(element => {
        try {
          // 텍스트 컨테이너 찾기
          const containers = window.DOMSelector.findTextContainers(element);
  ...
  function getStatistics() {
    ...
    if (checkDependencies()) {
      // 각 모듈에서 통계 수집
      if (window.DOMManipulator) {
        const manipulatorStats = window.DOMManipulator.getStatistics();
        Object.assign(stats, manipulatorStats);
      }
      
      if (window.BatchEngine) {
        const batchStats = window.BatchEngine.getStatus();
