// batch_engine.js - ES 모듈 방식으로 리팩토링
...
function dispatchProgressEvent(progress) {
  try {
    safeDispatchEvent('batchengine:progress', { progress });
  } catch (error) {
    console.warn(`[${APP_CONFIG.appName}] 진행 상태 이벤트 발행 오류:`, error);
  }
}
...
function processBatches(items, options = {}) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      console.log(`[${APP_CONFIG.appName}] 총 ${batches.length}개 배치로 처리 (항목 ${processItems.length}개)`);
...
function validateProcessingRequest(items) {
  ...
  if (!state.itemProcessor || typeof state.itemProcessor !== 'function') {
    console.error(`[${APP_CONFIG.appName}] 항목 처리 함수가 설정되지 않았습니다.`);
    return false;
  }
  
  // 이미 처리 중인 경우
  if (state.isProcessing) {
    console.warn(`[${APP_CONFIG.appName}] 이미 배치 처리가 진행 중입니다.`);
    return false;
  }
  
  // 유효한 항목 확인
  if (!Array.isArray(items) || items.length === 0) {
    console.warn(`[${APP_CONFIG.appName}] 처리할 항목이 없습니다.`);
    return false;
  }
...
function deduplicateItems(items, options) {
  ...
  console.log(`[${APP_CONFIG.appName}] 중복 제거: ${items.length} → ${uniqueItems.length}`);
...
function dispatchStartEvent(totalItems, batchCount) {
  try {
    safeDispatchEvent('batchengine:start', {
      total: totalItems,
      batches: batchCount
    });
  } catch (error) {
    console.warn(`[${APP_CONFIG.appName}] 시작 이벤트 발행 오류:`, error);
  }
}
...
async function processBatchGroups(batches, processedResults, options, isAborted) {
  for (let i = 0; i < batches.length; i += options.maxConcurrentBatches) {
    // 중단 확인
    if (isAborted()) {
      console.log(`[${APP_CONFIG.appName}] 배치 처리가 중단되었습니다.`);
      break;
    }
    ...
    try {
      ...
      console.error(`[${APP_CONFIG.appName}] 배치 그룹 처리 중 오류:`, error);
...
async function processBatch(batch, batchIndex, processedResults, options, isAborted) {
  try {
    ...
    state.activeBatches--;
    console.error(`[${APP_CONFIG.appName}] 배치 ${batchIndex} 처리 오류:`, error);
    throw error;
  }
}
  ...
  while (retryCount <= options.retryCount) {
    try {
      ...
      if (retryCount <= options.retryCount) {
        console.warn(`[${APP_CONFIG.appName}] 항목 처리 오류, ${retryCount}번째 재시도 중:`, error);
...
function handleBatchCompletion(results, batch, batchIndex, startTime, isAborted) {
  ...
  if (!isAborted()) {
    try {
      safeDispatchEvent('batchengine:batch-complete', {
        batchIndex,
      ...
      console.warn(`[${APP_CONFIG.appName}] 배치 완료 이벤트 발행 오류:`, error);
    }
  }
}
...
function handleBatchError(error, batch, batchIndex, processedResults) {
  console.error(`[${APP_CONFIG.appName}] 배치 ${batchIndex} 처리 중 오류:`, error);
...
function finishProcessing(results, totalOriginalItems) {
  ...
  console.log(`[${APP_CONFIG.appName}] 배치 처리 완료 (${totalTime}ms, ${summary.itemsPerSecond}항목/초)`);
...
function dispatchCompleteEvent(summary) {
  try {
    safeDispatchEvent('batchengine:complete', { summary });
  } catch (error) {
    console.warn(`[${APP_CONFIG.appName}] 완료 이벤트 발행 오류:`, error);
  }
}
...
function handleProcessingError(error, reject) {
  ...
  console.error(`[${APP_CONFIG.appName}] 배치 처리 실패:`, error);
  
  // 오류 이벤트 발행
  try {
    safeDispatchEvent('batchengine:error', {
      error: error,
      processed: state.totalProcessed,
      total: state.totalItems
    });
  } catch (eventError) {
    console.warn(`[${APP_CONFIG.appName}] 오류 이벤트 발행 오류:`, eventError);
  }
...
function handleInitializationError(error, reject) {
  state.isProcessing = false;
  console.error(`[${APP_CONFIG.appName}] 배치 처리 초기화 오류:`, error);
  reject(error);
}
...
function abort() {
  ...
  try {
    state.abortController.abort();
    console.log(`[${APP_CONFIG.appName}] 배치 처리 중단 요청됨`);
    
    // 중단 이벤트 발행
    try {
      safeDispatchEvent('batchengine:aborted', {
        processed: state.totalProcessed,
        total: state.totalItems
      });
    } catch (error) {
      console.warn(`[${APP_CONFIG.appName}] 중단 이벤트 발행 오류:`, error);
    }
    
    return true;
  } catch (error) {
    console.error(`[${APP_CONFIG.appName}] 배치 처리 중단 오류:`, error);
    return false;
  }
}
...
function clearCache() {
  try {
    ...
    console.log(`[${APP_CONFIG.appName}] 배치 처리 캐시 초기화됨`);
    return true;
  } catch (error) {
...
function logSettingsChanges(oldSettings) {
  ...
  if (Object.keys(changedSettings).length > 0) {
    console.log(`[${APP_CONFIG.appName}] BatchEngine 설정 변경:`, changedSettings);
  }
}
