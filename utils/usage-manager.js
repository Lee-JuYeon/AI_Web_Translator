// usage-manager.js - ES 모듈 방식으로 리팩토링
import { APP_CONFIG, safeDispatchEvent, getCurrentMonth } from '../config.js';

// 모듈 스코프에서 상태 관리 (전역 변수 최소화)
let usageManagerInitialized = false;

// 구독 등급별 월간 토큰 한도
const SUBSCRIPTION_LIMITS = APP_CONFIG.subscriptionLimits;
...
async function getCurrentSubscription() {
  ...
  if (!usageManagerInitialized) {
    console.log(`[${APP_CONFIG.appName}] UsageManager 초기화 중`);
    ...
    return new Promise((resolve) => {
      chrome.storage.sync.get('subscription', (data) => {
        if (chrome.runtime.lastError) {
          console.warn(`[${APP_CONFIG.appName}] 구독 정보 조회 오류:`, chrome.runtime.lastError);
          resolve('FREE');
          return;
        }
  ...
  try {
    return new Promise((resolve) => {
      chrome.storage.sync.get('usage', (data) => {
        if (chrome.runtime.lastError) {
          console.warn(`[${APP_CONFIG.appName}] 사용량 조회 오류:`, chrome.runtime.lastError);
        ...
        if (!data || !data.usage || !data.usage.month || data.usage.month !== currentMonth) {
          ...
          saveUsageToStorage(newUsage).catch(error => {
            console.error(`[${APP_CONFIG.appName}] 사용량 저장 오류:`, error);
          });
...
async function saveUsageToStorage(usage) {
  return new Promise((resolve) => {
    chrome.storage.sync.set({ usage }, () => {
      if (chrome.runtime.lastError) {
        console.warn(`[${APP_CONFIG.appName}] 사용량 저장 오류:`, chrome.runtime.lastError);
        resolve(false);
      } else {
        resolve(true);
      }
...
async function recordUsage(tokens) {
  try {
    ...
    if (saved) {
      // 이벤트 발생 (사용량 업데이트)
      safeDispatchEvent('usage:updated', {
        usage: newUsage,
        added: tokens
      });
    }
...
async function canTranslate(estimatedTokens) {
  try {
    ...
    // 한도 초과 시 이벤트 발행
    if (!isWithinLimit) {
      safeDispatchEvent('usage:limit-exceeded', {
        required: tokenCount,
...
async function getUsageStats() {
  try {
    ...
    // 이벤트 발생 (사용량 통계 준비됨)
    safeDispatchEvent('usage:stats-ready', { stats });
...
async function setSubscription(level) {
  try {
    ...
    if (!SUBSCRIPTION_LIMITS.hasOwnProperty(level)) {
      console.warn(`[${APP_CONFIG.appName}] 유효하지 않은 구독 등급:`, level);
      return false;
    }
    ...
    return new Promise((resolve) => {
      chrome.storage.sync.set({ subscription: level }, () => {
        if (chrome.runtime.lastError) {
          console.warn(`[${APP_CONFIG.appName}] 구독 저장 오류:`, chrome.runtime.lastError);
          resolve(false);
          return;
        }
        
        // 이벤트 발생 (구독 업데이트)
        safeDispatchEvent('subscription:updated', {
          subscription: level
        });
...
getUsageStats().catch(error => {
  console.error(`[${APP_CONFIG.appName}] 초기 사용량 캐싱 오류:`, error);
});
